...
def replace_text_in_docs(
    docs: list[str], replacements: list[tuple[str, str]], *, count: int = 0, flags: int = re.MULTILINE
) -> None:
    ...
    for doc in docs:
        ...
        original = text
        for pattern, value in replacements:
            text = re.sub(pattern, value, text, count=count, flags=flags)
        if text != original:
            file_path.write_text(text)
...
def update_snapshot_rows(next_snapshot: str) -> None:
    ...
    replace_text_in_docs(COMPATIBILITY_DOCS, [(SNAPSHOT_ROW_PATTERN, snapshot_row)])
