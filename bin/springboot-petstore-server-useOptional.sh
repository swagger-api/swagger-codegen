...
output="samples/server/petstore/springboot-useoptional"
# if you've executed sbt assembly previously it will use that instead.
export JAVA_OPTS="${JAVA_OPTS} -XX:MaxPermSize=256M -Xmx1024M -Dlogback.configurationFile=bin/logback.xml"
ags="$@ generate -i modules/swagger-codegen/src/test/resources/3_0_0/petstore.yaml -l spring -c bin/springboot-petstore-server-useOptional.json -o $output -DhideGenerationTimestamp=true"
...
rm -rf samples/server/petstore/springboot-useoptional/src/gen
