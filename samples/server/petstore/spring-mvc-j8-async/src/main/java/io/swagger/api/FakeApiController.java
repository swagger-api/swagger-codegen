...
public class FakeApiController implements FakeApi {
    ...
    public Callable<ResponseEntity<Client>> testClientModel(@ApiParam(value = "client model" ,required=true ) @RequestBody Client body) {
        // do some magic!
        return new Callable<ResponseEntity<Client>>() {
            @Override
            public ResponseEntity<Client> call() throws Exception {
                return new ResponseEntity<Client>(HttpStatus.OK);
            }
        };
    }

    public Callable<ResponseEntity<Void>> testEndpointParameters(@ApiParam(value = "None", required=true ) @RequestPart(value="number", required=true)  BigDecimal number,
    ...
    public Callable<ResponseEntity<Void>> testEnumParameters(@ApiParam(value = "Form parameter enum test (string array)" , allowableValues="GREATER_THAN, DOLLAR") @RequestPart(value="enumFormStringArray", required=false)  List<String> enumFormStringArray,
