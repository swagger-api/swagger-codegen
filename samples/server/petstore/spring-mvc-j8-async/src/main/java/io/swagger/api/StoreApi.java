...
public interface StoreApi {
    ...
    default CompletableFuture<ResponseEntity<Void>> deleteOrder(@Min(1L)@ApiParam(value = "ID of the order that needs to be deleted", required=true, allowableValues="range=[1, infinity]"
    ...
    @RequestMapping(value = "/store/order/{orderId}",
        ...
        method = RequestMethod.GET)
    default CompletableFuture<ResponseEntity<Order>> getOrderById(@Min(1L) @Max(10L) @ApiParam(value = "ID of pet that needs to be fetched", required=true, allowableValues="range=[1, 10]"
) @PathVariable("orderId") Long orderId) {
        if(getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
            if (getAcceptHeader().get().contains("application/json")) {
                try {
