...
public interface StoreApi {
    ...
    @RequestMapping(value = "/store/order/{orderId}",
        ...
        method = RequestMethod.GET)
    default CompletableFuture<ResponseEntity<Order>> getOrderById(@DecimalMin("1") @DecimalMax("5") @ApiParam(value = "ID of pet that needs to be fetched",required=true) @PathVariable("orderId") Integer orderId) {
        if(getObjectMapper().isPresent() && getAcceptHeader().isPresent()) {
        } else {
            log.warn("ObjectMapper or HttpServletRequest not configured in default StoreApi interface so no example is generated");
        }
