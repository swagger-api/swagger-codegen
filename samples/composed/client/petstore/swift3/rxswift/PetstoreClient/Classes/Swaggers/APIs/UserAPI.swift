...
open class UserAPI: APIBase {
  ...
  "username" : "username"
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func userUsernamePut(body: User, username: String, completion: @escaping ((_ error: ErrorResponse?) -> Void)) {
        userUsernamePutWithRequestBuilder(body: body, username: username).execute { (response, error) -> Void in
            completion(error)
        }
    }
    ...
    /**
     ...
     - returns: Observable<Void>
     */
    open class func userUsernamePut(body: User, username: String) -> Observable<Void> {
        return Observable.create { observer -> Disposable in
            userUsernamePut(body: body, username: username) { error in
                if let error = error {
                    observer.on(.error(error as Error))
                } else {
                    observer.on(.next())
    ...
    /**
     ...
     - returns: RequestBuilder<Void> 
     */
    open class func userUsernamePutWithRequestBuilder(body: User, username: String) -> RequestBuilder<Void> {
        var path = "/user/{username}"
        let usernamePreEscape = "\(username)"
        let usernamePostEscape = usernamePreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        path = path.replacingOccurrences(of: "{username}", with: usernamePostEscape, options: .literal, range: nil)
