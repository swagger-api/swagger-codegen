...
# these are needed for httpsig aka  
...
class ApiClient(object):
    ...
    def __init__(self, configuration=None, header_name=None, header_value=None,
        ...
        self.private_key = None # placeholder for cache of parsed key file
    ...
    def update_params_for_auth(self, headers, querys, auth_settings, resource_path, body, method, url):
        ...
        for auth in auth_settings:
            ...
            if auth_setting:
                ...
                elif auth_setting['in'] == 'httpsig': # https://datatracker.ietf.org/doc/draft-cavage-http-signatures/?include_text=1
                    self.prepare_auth_header(auth_setting, resource_path, method, body, querys, url)
                    headers.update(self.default_headers) # headers may have changed, canonicalize
                else:
                    raise ValueError(
    ...
    def get_rsasig_b64encode(self, auth_settings, digest):
        """
        :param self.private_key : previously loaded and cached RSA key object. 
        :param auth_settings['private_key_path'] : abs path to private key .pem file.
        ...
        if not self.private_key:
        	try:
        		key = open(auth_settings['private_key_path'], "r").read()
        		rsakey = RSA.importKey(key)
        	except IOError as e:
        		print "I/O error reading key file {2} ({0}): {1}".format(e.errno, e.strerror, auth_settings['private_key_path'])	
        		raise
        	except: 
        		print "Error with key file {2} ({0}): {1}".format(e.errno, e.strerror, auth_settings['private_key_path'])
        		raise
        	self.private_key = rsakey
        	
        rsakey = self.private_key
    ...
    def prepare_str_to_sign(self, req_tgt, hdrs):
        ...
        """
        ss = []
        ss.append("(request-target): {0}".format(req_tgt.lower()))
    
        for key, value in hdrs.items():
            ss.append("{0}: {1}".format(key.lower(),value))
        
        return '\n'.join(ss)

    def get_auth_header(self, auth_settings, hdrs, signed_msg):
        """
        ...
        auth_str = auth_str + " " + "keyId=\"" + auth_settings['api_key_id'] + "\"," + "algorithm=\"" + auth_settings['digest_algorithm'] + "\"," + "headers=\"(request-target)"
    ...
    def prepare_auth_header(self, auth_settings, resource_path, method, body, query_params, url):
        """
        ...
        :param auth_setting: Authsettings dictionary
        ...
        target_host = parsedUrl.netloc
        target_path = parsedUrl.path
        
        request_target = method + " " + target_path
        ...
        if auth_settings['digest_algorithm'] != 'rsa-sha256':
                raise ValueError('only rsa-sha256 supported for digest_algorithm in httpsig')
        digest = self.get_sha256_digest(string_to_sign.encode())
        
        b64_signed_msg = self.get_rsasig_b64encode(auth_settings, digest)
        
        auth_header = self.get_auth_header(auth_settings, headers, b64_signed_msg)
        
        self.set_default_header('Date', str(cdate))
        self.set_default_header('Host', str(target_host))
        self.set_default_header('Digest', 'SHA-256={0}'.format(b64_body_digest.decode('ascii')))
        self.set_default_header('Authorization', str(auth_header))
