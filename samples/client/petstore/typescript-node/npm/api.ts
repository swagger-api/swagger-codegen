...
function parseData(data, type) {
    if (data instanceof Array) {
        ...
        return getInstanceFromTypeName(data, type);
    }
}

function parseArray(content, type) {
    // Array<XType> => XType
    type = type.replace("Array<", ""); 
    type = type.substring(0, type.length - 1);
    
    var result = new Array(content.length);
    for (let i = 0; i < content.length; i++) {
        result[i] = parseData(content[i], type)
    }
    return result;
}

var typeToInstanceMap = {
    "Category": (data) => new Category(data),
    "Order": (data) => new Order(data),
    "Pet": (data) => new Pet(data),
    "Tag": (data) => new Tag(data),
...
function getInstanceFromTypeName(data, type) {
    ...
    if (typeToInstanceMap[type]) {
        return typeToInstanceMap[type](data);
    }
    console.log("Couldn't find proper instance for type " + type + ". Not transforming it");
    return data;
}
...
export class Category {
    ...
    constructor(data?) {
        if (data) {
                if ("id" in data) {    
                    this.id = parseData(data["id"], "number");
                } else {
                    this.id = undefined;
                }
                if ("name" in data) {    
                    this.name = parseData(data["name"], "string");
                } else {
                    this.name = undefined;
                }
        }
    ...
    */
    ...
    constructor(data?) {
        if (data) {
                ...
                if ("petId" in data) {    
                    this.petId = parseData(data["petId"], "number");
                } else {
                    this.petId = undefined;
                }
                if ("quantity" in data) {    
                    this.quantity = parseData(data["quantity"], "number");
                } else {
                    this.quantity = undefined;
                }
                if ("shipDate" in data) {    
                    this.shipDate = parseData(data["shipDate"], "Date");
                } else {
                    this.shipDate = undefined;
                }
                if ("status" in data) {    
                    this.status = parseData(data["status"], "string");
                } else {
                    this.status = undefined;
                }
                if ("complete" in data) {    
                    this.complete = parseData(data["complete"], "boolean");
                } else {
    ...
    */
    ...
    constructor(data?) {
        if (data) {
                ...
                if ("category" in data) {    
                    this.category = parseData(data["category"], "Category");
                } else {
                ...
                if ("photoUrls" in data) {    
                    this.photoUrls = parseData(data["photoUrls"], "Array&lt;string&gt;");
                } else {
                    this.photoUrls = undefined;
                }
                if ("tags" in data) {    
                    this.tags = parseData(data["tags"], "Array&lt;Tag&gt;");
                } else {
                    this.tags = undefined;
                }
                if ("status" in data) {    
    ...
    */
    ...
    constructor(data?) {
        if (data) {
                ...
                if ("username" in data) {    
                    this.username = parseData(data["username"], "string");
                } else {
                    this.username = undefined;
                }
                if ("firstName" in data) {    
                    this.firstName = parseData(data["firstName"], "string");
                } else {
                    this.firstName = undefined;
                }
                if ("lastName" in data) {    
                    this.lastName = parseData(data["lastName"], "string");
                } else {
                    this.lastName = undefined;
                }
                if ("email" in data) {    
                    this.email = parseData(data["email"], "string");
                } else {
                    this.email = undefined;
                }
                if ("password" in data) {    
                    this.password = parseData(data["password"], "string");
                } else {
                    this.password = undefined;
                }
                if ("phone" in data) {    
                    this.phone = parseData(data["phone"], "string");
                } else {
                    this.phone = undefined;
                }
                if ("userStatus" in data) {    
                    this.userStatus = parseData(data["userStatus"], "number");
                } else {
        ...
        return new Promise<{ response: http.ClientResponse; body: { [key: string]: number; };  }>((resolve, reject) => {
            request(requestOptions, (error, response, body) => {
                if (error) {
                    ...
                    if (response.statusCode >= 200 && response.statusCode <= 299) {
                        body = parseData(body, "{ [key: string]: number; }");
