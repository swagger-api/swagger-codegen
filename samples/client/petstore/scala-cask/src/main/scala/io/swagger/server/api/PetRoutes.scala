...
final case class PetRoutes(service : PetService) extends cask.Routes {
    ...
    def routeWorkAroundForGETPet(request: cask.Request,status : Seq[String] = Nil, tags : Seq[String] = Nil) = {
        ...
        val serviceResponse =         for {
            ...
            body <- Parsed.eval(Pet.fromJsonString(request.bodyAsString))
            result <- Parsed.eval(service.addPet(body))
        } yield result
        ...
        val serviceResponse =         for {
            ...
            apiKey <- request.headerSingleValue("apiKey", false)
        ...
        val serviceResponse =         for {
            ...
            /** TODO - this is a bit of a hack - we should do content type negotiation */
            body <- Parsed.eval(Pet.fromJsonString(request.bodyAsString))
            result <- Parsed.eval(service.updatePet(body))
        } yield result
    ...
    name <- request.formSingleValue("name", false)
    
    status <- request.formSingleValue("status", false)
    ...
    additionalMetadata <- request.formSingleValue("additionalMetadata", false)
    
    file <- request.formValueAsFile("file", false)
