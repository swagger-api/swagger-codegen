...
module SwaggerClient
  class PetApi
    ...
    # @param [Hash] opts the optional parameters
    # @option opts [Array<String>] :status Status values that need to be considered for filter (default to available)
    # @return [Array<Pet>]
    ...
    def find_pets_by_status_with_http_info(opts = {})
      ...
      if opts[:'status'] && !opts[:'status'].all?{|item| ['available', 'pending', 'sold'].include?(item)}
        fail ArgumentError, 'invalid value for "status", must include one of available, pending, sold'
      end
      # resource path
