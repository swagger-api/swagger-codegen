...
open class StoreAPI: APIBase {
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func getOrderById(orderId: Int32, completion: @escaping ((_ data: Order?, _ error: ErrorResponse?) -> Void)) {
     ...
     */
    open class func getOrderById(orderId: Int32) -> Observable<Order> {
        return Observable.create { observer -> Disposable in
     ...
     */
    open class func getOrderByIdWithRequestBuilder(orderId: Int32) -> RequestBuilder<Order> {
        var path = "/store/order/{order_id}"
        let orderIdPreEscape = "\(orderId)"
        let orderIdPostEscape = orderIdPreEscape.addingPercentEncoding(withAllowedCharacters: .urlPathAllowed) ?? ""
        path = path.replacingOccurrences(of: "{order_id}", with: orderIdPostEscape, options: .literal, range: nil)
