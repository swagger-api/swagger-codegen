...
open class UserAPI: APIBase {
  ...
  "username" : "username"
    ...
    /**
     ...
     - parameter completion: completion handler to receive the data and the error objects
     */
    open class func updateUser(body: User, username: String, completion: @escaping ((_ error: ErrorResponse?) -> Void)) {
    ...
    /**
     ...
     - returns: Observable<Void>
     */
    open class func updateUser(body: User, username: String) -> Observable<Void> {
        return Observable.create { observer -> Disposable in
            updateUser(body: body, username: username) { error in
                if let error = error {
                    observer.on(.error(error as Error))
                } else {
                    observer.on(.next())
