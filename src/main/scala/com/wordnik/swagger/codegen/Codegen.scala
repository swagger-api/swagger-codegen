...
class Codegen(config: CodegenConfig) {
  ...
  def generateSource(bundle: Map[String, AnyRef]): String = {
    ...
    val template = engine.compile(
      TemplateSource.fromText(config.templateDir + "/uber-model.mustache",
        Source.fromFile(config.templateDir + "/uber-model.mustache").mkString))
    ...
    var output = engine.layout(config.templateDir + "/uber-model.mustache", template, data.toMap)
    output
  }
