...
public class OnlineGeneratorOptionsTest {
	...
	public void generateOptionsTest(final OptionsProvider provider) throws ApiException, IOException {
		...
		final Maps.EntryTransformer<String, InvocationCounter, String> transformer =
			...
			@Override
			public String transformEntry(final String key, final InvocationCounter value) {
	...
	private HashMap<String, InvocationCounter> convertOptions(final OptionsProvider provider) {
	...
	private static String loadClassResource(final Class<?> cls, final String name) throws IOException {
		InputStream in = null;
		try {
	...
	private static class InvocationCounter {
		...
		public InvocationCounter(final String value) {
			this.value = value;
		}
	...
	@Test(dataProvider = OPTIONS_PROVIDER)
	public void getOptionsTest(final OptionsProvider provider) throws ApiException {
		...
		final Function cliOptionWrapper = new Function<CliOption, CliOptionProxy>() {
			@Nullable
			@Override
			public CliOptionProxy apply(@Nullable final CliOption option) {
	...
	private static class CliOptionProxy {
		...
		public CliOptionProxy(final CliOption wrapped){
