...
public class BashTest {
    ...
    public void petstoreOperationTest() {
        ...
        final Swagger swagger =
                new SwaggerParser().read("src/test/resources/2_0/petstore-bash.json");
        final DefaultCodegen codegen = new BashClientCodegen();
        final Operation findPetsByStatusOperation = swagger.getPath("/pet/findByStatus").getGet();

        final CodegenOperation op =
                codegen.fromOperation("/pet/findByStatus", "GET", findPetsByStatusOperation,
                        swagger.getDefinitions(), swagger);

        Assert.assertTrue(op.vendorExtensions.containsKey("x-bash-codegen-sample"));

        Assert.assertEquals(op.vendorExtensions.get("x-bash-codegen-description"),
    ...
    public void petstoreParameterExampleTest() {
        ...
        final Swagger swagger =
                new SwaggerParser().read("src/test/resources/2_0/petstore-bash.json");
        final DefaultCodegen codegen = new BashClientCodegen();
        final Operation addPetOperation = swagger.getPath("/pet").getPost();

        final CodegenOperation op =
                codegen.fromOperation("/pet", "POST", addPetOperation, swagger.getDefinitions(),
                        swagger);
    ...
    public void escapeTextTest() {
        ...
        Assert.assertEquals(codegen.escapeText("__Bold text__"), "__Bold text__");

        Assert.assertEquals(codegen.escapeText("**Bold text**"), "**Bold text**");

        Assert.assertEquals(codegen.escapeText("*Italic text*"), "*Italic text*");

        Assert.assertEquals(codegen.escapeText("_Italic text_"), "_Italic text_");
    ...
    public void escapeUnsafeCharactersTest() {
        ...
        Assert.assertEquals(codegen.escapeUnsafeCharacters("`no backticks`"), "'no backticks'");
